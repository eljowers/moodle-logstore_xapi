...
class Controller extends PhpObj {
    ...
    public static $routes = [
        ...
        // '\mod_quiz\event\question_manually_graded' => 'QuestionGraded',
        '\core\event\user_loggedin' => 'Event',
        '\core\event\user_loggedout' => 'Event',
        '\core\event\user_graded' => 'AssignmentGraded',
        ...
        foreach ($events as $index => $opts) {
            ...
            if (isset(static::$routes[$route]) && ($opts['userid'] > 0 || $opts['relateduserid'] > 0)) {
                try {
                    ...
                    echo "<script type='text/javascript'> console.debug('ERROR IN EXPANDER: ','".json_encode(print_r($e, true))."');</script>";
